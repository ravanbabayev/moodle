...
$PAGE->set_url('/local/lidio/merchant.php');
...
if ($data = data_submitted() and confirm_sesskey()) {
    if (isset($data->submitbutton)) {
        ...
        $merchantdata->email = required_param('email', PARAM_EMAIL);
        ...
        if (empty($merchantdata->email) || !validate_email($merchantdata->email)) {
...
$templatecontext = [
    'sesskey' => sesskey(),
    'actionurl' => $CFG->wwwroot . '/local/lidio/merchant.php',
    ...
    'payment_methods_options' => [
        ...
        ['value' => 'mobile_payment', 'text' => get_string('mobilepayment', 'local_lidio'), 
            'checked' => (isset($merchant) && strpos($merchant->payment_methods, 'mobile_payment') !== false)],
        ['value' => 'other', 'text' => get_string('otherpayment', 'local_lidio'), 
            'checked' => (isset($merchant) && strpos($merchant->payment_methods, 'other') !== false)]
    ],
    'merchant' => $merchant,
...
} else if ($merchant && $merchant->status === 'pending' && !$edit) {
